...
function sub2!(a::Vector{ArbDigit}, b::Vector{ArbDigit})
    len = min(length(a), length(b))
    a_lo, a_hi = @views a[1:len], a[len+1:end]
    b_lo, b_hi = @views b[1:len], b[len+1:end]
...
Base.:(-)(a::ArbUInt, b::T) where T <: Unsigned = sub!(deepcop(a), b)
Base.:(-)(a::T, b::ArbUInt) where T <: Unsigned = sub!(deepcop(b), a)